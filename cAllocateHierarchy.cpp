	...
	LPD3DXMESHCONTAINER *ppNewMeshContainer)
{
	...
	if (pMeshData && pMeshData->pMesh)
	{
		...
		DWORD test1 = pMeshData->pMesh->GetNumVertices();
		DWORD test2 = D3DXGetFVFVertexSize(pMeshData->pMesh->GetFVF());

		HRESULT hr = D3DXComputeBoundingBox((D3DXVECTOR3*)pV,
			/*pMeshData->pMesh->GetNumVertices()*/test1,
			/*D3DXGetFVFVertexSize(pMeshData->pMesh->GetFVF())*/test2,
			&vMin,
			&vMax
			);
